...
user_preference_allow_ajax_update('activity_aside_state', PARAM_ALPHA);
$isfolded = 0;
...
if (isloggedin()) {
    ...
    if (isset($PAGE->cm->id) && in_array($PAGE->cm->modname, $activities)) {
        $aside_right_state = get_user_preferences('activity_aside_state', '');
    }else{
        set_user_preference('activity_aside_state', '');
    }
} else {
    $menubar_state = 'fold';
    $isfolded = 1;
